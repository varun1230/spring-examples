...
public class EmplyeeController {
    ...
    public String registration(@ModelAttribute("employeeForm") Employee employeeForm, BindingResult bindingResult, Model model, HttpServletRequest req) {
        ...
        return "redirect:/welcome/"+employeeForm.getId();
    }

	@RequestMapping(value = {"/", "/welcome/{id}"}, method = RequestMethod.GET)
    public String welcome(@PathVariable("id") long id,  Model model, HttpServletRequest req) {
        
		Employee empForm = employeeService.findById(id);
